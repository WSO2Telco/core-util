...
public class OauthAdminServiceClientStubFactory extends BasePoolableObjectFactory {

    private static final Logger log = LoggerFactory.getLogger(OauthAdminServiceClientStubFactory.class);
    AdministrationServiceConfig config;
    private HttpClient httpClient;

    public OauthAdminServiceClientStubFactory() {
        this.config = AdministrationServiceConfig.getInstance();
        this.httpClient = createHttpClient();
    }

    /**
     * This creates an OAuth2TokenValidationServiceStub object to the pool.
     *
     ...
     */
    public OAuthAdminServiceStub generateStub() throws SpProvisionServiceException {
        OAuthAdminServiceStub stub;
        try {
            URL hostURL = new URL(config.getAdminServicesHostUrl());
